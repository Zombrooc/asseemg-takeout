...
impl TakeoutRepository {
  ...
  pub fn confirm_atomic(
    ...
    let result = {
      match conn.query_row("SELECT status FROM takeout_events WHERE request_id = ?1", [request_id], |r| r.get::<_, String>(0)) {
        Ok(_) => {
      ...
      match conn.query_row("SELECT request_id FROM check_ins WHERE ticket_id = ?1", [ticket_id], |r| r.get::<_, String>(0)) {
        Ok(existing_request_id) => {
          let _ = conn.execute("ROLLBACK", []);
          return Ok(ConfirmAtomicResult::Conflict { existing_request_id });
        }
        Err(rusqlite::Error::QueryReturnedNoRows) => {}
        Err(e) => return Err(e),
      }
...
mod tests {
  ...
  fn delete_by_event_id_removes_only_events_for_that_event() {
    ...
    let remaining: String = conn
      .query_row("SELECT ticket_id FROM takeout_events LIMIT 1", [], |r| r.get(0))
      .unwrap();
