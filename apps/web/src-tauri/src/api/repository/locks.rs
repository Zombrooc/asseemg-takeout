...
impl LocksRepository {
  ...
  pub fn acquire(pool: &DbPool, participant_id: &str, device_id: &str) -> Result<AcquireResult, rusqlite::Error> {
  ...
  pub fn renew(pool: &DbPool, participant_id: &str, device_id: &str) -> Result<bool, rusqlite::Error> {
    let conn = pool.conn.lock().map_err(|_| rusqlite::Error::InvalidParameterName("lock".into()))?;
    let now = Self::now_secs();
    let expires_at = now + TTL_SECS;
    let n = conn.execute(
  ...
  pub fn release(pool: &DbPool, participant_id: &str, device_id: Option<&str>) -> Result<u64, rusqlite::Error> {
    let conn = pool.conn.lock().map_err(|_| rusqlite::Error::InvalidParameterName("lock".into()))?;
    let n = if let Some(did) = device_id {
      conn.execute("DELETE FROM locks WHERE participant_id = ?1 AND device_id = ?2", params![participant_id, did])?
    } else {
      conn.execute("DELETE FROM locks WHERE participant_id = ?1", params![participant_id])?
    };
    Ok(n as u64)
  }

  pub fn get_holder(pool: &DbPool, participant_id: &str) -> Result<Option<(String, i64)>, rusqlite::Error> {
    let conn = pool.conn.lock().map_err(|_| rusqlite::Error::InvalidParameterName("lock".into()))?;
    let now = Self::now_secs();
