...
impl PairingRepository {
  ...
  pub fn insert_token(pool: &DbPool, token: &str, expires_at: &str) -> Result<(), rusqlite::Error> {
    let conn = pool.conn.lock().map_err(|_| rusqlite::Error::InvalidParameterName("lock".into()))?;
    conn.execute("INSERT OR REPLACE INTO pairing_tokens (token, expires_at) VALUES (?1, ?2)", params![token, expires_at])?;
    Ok(())
  }

  pub fn consume_token(pool: &DbPool, token: &str) -> Result<bool, rusqlite::Error> {
    let conn = pool.conn.lock().map_err(|_| rusqlite::Error::InvalidParameterName("lock".into()))?;
    let n = conn.execute("DELETE FROM pairing_tokens WHERE token = ?1", params![token])?;
    Ok(n > 0)
  }
  ...
  pub fn find_device_by_token(
    ...
    let conn = pool.conn.lock().map_err(|_| rusqlite::Error::InvalidParameterName("lock".into()))?;
    let mut stmt = conn.prepare("SELECT device_id FROM paired_devices WHERE access_token = ?1")?;
