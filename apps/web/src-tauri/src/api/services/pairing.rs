...
impl PairingService {
  pub fn get_info(pool: Arc<DbPool>, base_url: String) -> Result<ConnectionInfo, String> {
    if let Some((token, expires_at)) = PairingRepository::get_current_token(&pool).map_err(|e| e.to_string())? {
  ...
  pub fn pair(
    ...
    let consumed = PairingRepository::consume_token(&pool, &pairing_token).map_err(|e| e.to_string())?;
    if !consumed {
      return Err("invalid or expired pairing token".to_string());
    }
    let access_token = uuid::Uuid::new_v4().to_string();
    PairingRepository::insert_device(&pool, &device_id, &access_token).map_err(|e| e.to_string())?;
    Ok(access_token)
  }
...
mod tests {
  ...
  fn get_info_creates_token_when_none() {
    let pool = mem_pool();
    let info = PairingService::get_info(pool.clone(), "http://localhost:5555".to_string()).unwrap();
